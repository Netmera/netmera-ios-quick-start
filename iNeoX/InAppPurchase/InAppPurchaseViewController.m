...
@property (nonatomic, strong) ProductsListViewController *productsList;
@property (nonatomic, strong) PurchasesListViewController *purchasesList;
...
- (instancetype)init
{
  ...
  InAppPurchaseViewController *vc = [sb instantiateViewControllerWithIdentifier:@"ParentViewControllerID"];
  self = vc;
  if (self) {
  }
  return self;
...
- (void)viewDidLoad
{
    ...
    // Get the storyboard
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"InAppPurchase" bundle:nil];
...
-(NSMutableArray *)dataSourceForPurchasesUI
{
    ...
    {
        dataSource = [[NSMutableArray alloc] initWithObjects:[[PurchaseModel alloc] initWithName:@"PURCHASED" elements:[StoreObserver sharedInstance].productsPurchased],
                                                             [[PurchaseModel alloc] initWithName:@"RESTORED" elements:[StoreObserver sharedInstance].productsRestored],nil];
    }
    else if (self.restoreWasCalled && [[StoreObserver sharedInstance] hasRestoredProducts])
    {
        dataSource = [[NSMutableArray alloc] initWithObjects:[[PurchaseModel alloc] initWithName:@"RESTORED" elements:[StoreObserver sharedInstance].productsRestored], nil];
    }
    else if ([[StoreObserver sharedInstance] hasPurchasedProducts])
    {
        dataSource = [[NSMutableArray alloc] initWithObjects:[[PurchaseModel alloc] initWithName:@"PURCHASED" elements:[StoreObserver sharedInstance].productsPurchased], nil];
    }
