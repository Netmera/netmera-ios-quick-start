...
#import "PurchasesListViewController.h"
#import "PaymentTransactionDetails.h"

@interface PurchasesListViewController()
@property (nonatomic, strong) NSMutableArray *allPurchases;

@end


@implementation PurchasesListViewController
...
{
    PurchaseModel *model = (self.allPurchases)[section];
    return model.name;
}
...
{
    PurchaseModel *model = (self.allPurchases)[indexPath.section];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    PurchaseModel *model = self.allPurchases[selectedRowIndex];
    ...
    if ([segue.identifier isEqualToString:@"showPaymentTransaction"])
    {
        ...
        [purchaseDetails addObject:[[PurchaseModel alloc] initWithName:@"PRODUCT IDENTIFIER" elements:@[paymentTransaction.payment.productIdentifier]]];
        [purchaseDetails addObject:[[PurchaseModel alloc] initWithName:@"TRANSACTION ID" elements:@[paymentTransaction.transactionIdentifier]]];
        [purchaseDetails addObject:[[PurchaseModel alloc] initWithName:@"TRANSACTION DATE" elements:@[[[self dateFormatter] stringFromDate:paymentTransaction.transactionDate]]]];
        ...
        if (allDownloads.count > 0)
        {
            ...
            [purchaseDetails addObject:[[PurchaseModel alloc] initWithName:@"DOWNLOAD" elements:@[identifier,version,contentLength]]];
        }
        ...
        if (paymentTransaction.originalTransaction !=nil)
        {
            ...
            [purchaseDetails addObject:[[PurchaseModel alloc] initWithName:@"ORIGINAL TRANSACTION" elements:[NSMutableArray arrayWithObjects:transactionID,transactionDate, nil]]];
        }
